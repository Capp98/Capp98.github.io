...
function createViews() {
    ...
    json.map(planta => {
        ...
        tipo = tipo.concat(`${planta.iluminacao} ${planta.rega}`).trim()
        article.setAttribute('tipo', tipo)
        ...
        divWater.className = 'rega'

        span = document.createElement('span');
        h4 = document.createElement('h4');
        h4.innerHTML = 'Rega'
        span.className = planta.rega
        divInfos.appendChild(h4)
        divInfos.appendChild(span)
...
function show(element) {
    let container = document.getElementsByTagName('aside')[0]
    container.classList.toggle('on')
    let a = container.children[0].children[0]
    a.innerHTML = container.classList.contains('on') ?
        '<' : '>'
}
...
function searchByName() {
    ...
    let plants = document.getElementById('plantas').children;
...
function showAllPlants(plants) {
    ...
    plants.forEach(view => view.style.display = "initial");
}

function showPlantsByNameAndTags(plants, name, tags) {
    let tagcollection = ''
    tags.forEach(tag => tagcollection += `${tag.id} `)
    tagcollection = tagcollection.trim().split(' ')

    for (i = 0; i < plants.length; i++) {

        let atributos = plants[i].attributes['tipo'].value.split(' ')
        ...
        if (!(plants[i].className.toLowerCase().includes(name.toLowerCase())
            ...
            plants[i].style.display = "initial";
        }
    }
}

function showPlantsByTags(plants, tags) {
    let tagcollection = ''
    tags.forEach(tag => tagcollection += `${tag.id} `)
    tagcollection = tagcollection.trim().split(' ')
    ...
    for (i = 0; i < plants.length; i++) {
        ...
        if (!(atributos.length === tagcollection.length)) {
            plants[i].style.display = "none";
        }
        else {
            plants[i].style.display = "initial";
...
function searchByTag(evt) {
    evt.classList.toggle('selecionado')
    ...
    if (tags.length === 0 && name === '') {
        ...
        showPlantsByTags(plants, tags)
    }
}

// function maximize(evt) {
//     let article = evt.currentTarget
//     console.log(article);
